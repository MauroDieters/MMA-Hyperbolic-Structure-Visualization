...
def compute_cone_wedge_path(
    ...
    cx, cy = float(anchor_2d[0]), float(anchor_2d[1]) # anchor's coords
    anchor_norm = np.sqrt(cx**2 + cy**2) # distance from origin

    if anchor_norm < 1e-8:
        theta_center = 0.0
    else:
        theta_center = np.arctan2(cy, cx) # direction from origin to anchor

    half_aperture = np.deg2rad(min(aperture_deg / 2.0, 89.0))
    # Left/Right boundary angles
...
def compute_cone_highlights_512d(
    ...
    anchor = hd_lorentz[anchor_idx] # anchor point
...
def cone_wedge_polygon(
    ...
    """
    Returns points instead of SVG string (like the compute_cone_wedge_path)
